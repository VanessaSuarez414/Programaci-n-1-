...
while True:
    ...
    print("8. tangente")
    print("9. Salir")
    op = int(input("Ingrese la operación que desea realizar (1-9): "))
    if op == 1:
        ...
        if numero1 > 0:
            rta = math.sqrt
            print(f"La raíz cuadrada de {numero1} es: {rta}")
        else:
            print("Error!!! No se puede calcular la raiz de numeros negativos")    
    elif op == 7:
        numero1 = float(input("Ingrese el ángulo en grados: "))
        rta = math.cos(math.radians(numero1))
        print(f"El coseno de {numero1}° es: {rta}")
    elif op == 8:
        numero1 = float(input("Ingrese el ángulo en grados: "))
        rta = math.tan(math.radians(numero1))
        print(f"La tangente de {numero1}° es: {rta}")
    elif op ==9:
        break
    else:
